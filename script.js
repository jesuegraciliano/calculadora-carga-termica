...
// Mapeamento para nomes amigáveis na exibição e valores padrão (da sua primeira tabela)
...
function generateInputRows() {
    for (const key in PARCEL_DEFINITIONS) {
        ...
        calculatedLoadDiv.textContent = '0.00'; // Valor inicial
...
function displayResults(results) {
    ...
    // Atualiza as cargas individuais na tabela
    for (const key in results.individualLoads) {
        const span = calculatedLoadSpans[key];
        if (span) {
            span.textContent = results.individualLoads[key].toFixed(2);
        }
    }
}
...
function calculateAndDisplayAll() {
    ...
    if (inputs) {
        ...
        totalKcalhSpan.textContent = '0.00';
        totalBtuhSpan.textContent = '0.00';
        totalTrSpan.textContent = '0.00';
        for (const key in calculatedLoadSpans) {
            calculatedLoadSpans[key].textContent = '0.00';
        }
    }
}
